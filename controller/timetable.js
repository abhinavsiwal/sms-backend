              ...
              if (err) {
                ...
                if (!period_details || period_details.length == 0) {
                  ...
                  for (var i = 0; i < period_details.length; i++) {
                    var start = new Date(
                      "2022-12-12 " + period_details[i].start
                    );
                    var end = new Date("2022-12-12 " + period_details[i].end);
                    if (
                      (common.changeDateFormat(start) ==
                        common.changeDateFormat(current_start) &&
                        common.changeDateFormat(end) ==
                          common.changeDateFormat(current_end)) ||
                      ...
                      .then((time_table_details, err) => {
                        if (err) {
                          ...
                          if (time_table_details.length > 0) {
                            asyncLoop(
                              time_table_details,
                              function (item_new, next_new) {
                                // It will be executed one by one
                                ClassTimeTable.find({
                                  period_id: ObjectId(item_new.period_id),
                                  day: item.day,
                                  is_deleted: "N",
                                })
                                  .sort({ min: 1 })
                                    ...
                                    if (err) {
                                      ...
                                      if (period_details_new.length > 0) {
                                        var current_start = new Date(
                                          "2022-12-12 " + period_details.start
                                        );
                                        var current_end = new Date(
                                          "2022-12-12 " + period_details.end
                                        );
                                        for (
                                          var i = 0;
                                          i < period_details_new.length;
                                          i++
                                        ) {
                                          var start = new Date(
                                            "2022-12-12 " +
                                              period_details_new[i].start
                                          );
                                          var end = new Date(
                                            "2022-12-12 " +
                                              period_details_new[i].end
                                          );
                                          if (
                                            item.period_id.toString() !=
                                              item_new._id.toString() &&
                                            ((common.changeDateFormat(start) ==
                                              common.changeDateFormat(
                                                current_start
                                              ) &&
                                              common.changeDateFormat(end) ==
                                                common.changeDateFormat(
                                            ...
                                            return res.status(400).json({
                                              err:
                                                item_new.first_name +
                                                " already have another period please select diffrent teacher",
                                            });
                                            break;
                                          }
                                        }
                                  ...
                                  .then((class_time_table_details, err) => {
                                    if (err) {
                                      ...
                                      if (!class_time_table_details) {
                                        ...
                                        var period_data = new ClassTimeTable(
                                          params
                                        );
                                        period_data.save(function (
                                          err,
                                          result
                                        ) {
                                          if (err) {
...
exports.timeTableListV2 = (req, res) => {
  try {
    ...
    if (common.checkValidationRulesJson(req.body, res, rules)) {
      ...
      PeriodMaster.find(params)
        ...
        .then((result, err) => {
          result.sort((a, b) => {
            return (
              new Date("12-12-2022 " + a.start) -
              new Date("12-12-2022 " + b.start)
            );
          });
          var output = {};
          var days = [
          ...
          ClassTimeTable.find(params)
            ...
            .exec((err, result_t) => {
              if (err) {
                console.log(err);
                return common.sendJSONResponse(
                  res,
                  0,
                  "Problem in fetching timetable. Please try again.",
                  null
                );
              } else {
                days.forEach((day) => {
                  ...
                  result.forEach((re) => {
                    ...
                    result_t.forEach((rt) => {
                      if (
                        rt.day == day &&
                        rt.period_id.toString() == re._id.toString() &&
                        rt.staff &&
                        rt.staff._id
                      ) {
                        ...
                        output[day].push({
                          ...
                          meet_link:
                            rt.meet_link !== undefined ? rt.meet_link : "",
                          staff: {
                            _id: rt.staff._id,
    ...
    return common.sendJSONResponse(
      res,
      0,
      "Can't Able To fetch student list",
      null
    );
  }
};
      ...
      ClassTimeTable.find(params)
        ...
        .exec((err, result_t) => {
          if (err) {
            ...
            result_t.sort((a, b) => {
              return (
                new Date("12-12-2022 " + a.period_id.start) -
                new Date("12-12-2022 " + b.period_id.start)
              );
            });
            return common.sendJSONResponse(
              res,
              1,
              "Period list fetched successfully.",
              result_t
            );
          }
        });
