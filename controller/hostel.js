    ...
    form.parse(req, (err, fields, file) => {
        if (err) {
            ...
            if ((fields.student && (!fields.class || !fields.section)) || (fields.department && !fields.staff)) {
                ...
                if (fields.student){
                    HostelRoomAllocation.findOne({ student: ObjectId(fields.student) })
                        .then((result, err) => {
                            if (err) {
                                console.log(err);
                                return res.status(400).json({
                                    err: "Problem in alloting room. Please try again.",
                                });
                            } else if (result) {
                                return res.status(400).json({
                                    err: "Room is already alloted to the selected student.",
                                });
                            }
                        });
                }
                ...
                if (fields.staff){
                    HostelRoomAllocation.findOne({ staff: ObjectId(fields.staff) })
                        .then((result, err) => {
                            if (err) {
                                console.log(err);
                                return res.status(400).json({
