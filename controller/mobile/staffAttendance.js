    ...
    Attendance.find({
      ...
      .then(async (attendance, err) => {
        ...
        await attendance.map(async (attendance_data) => {
          ...
          if (
            new Date(attendance_data.date).setHours(0,0,0) >= new Date(start_date).setHours(0,0,0) &&
            new Date(attendance_data.date).setHours(0,0,0) <= new Date(end_date).setHours(0,0,0)
            ) {
              console.log("here");
...
exports.editAttendanceForDate=async(req,res)=>{
  let form = new formidable.IncomingForm();
  form.keepExtensions = true;
  form.parse(req, async (err, fields) => {
    console.log(fields);
